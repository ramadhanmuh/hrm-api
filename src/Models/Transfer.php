...
class Transfer extends Model
{
    static $table = 'transfers',
            $columns = 'id, employeeId, designationId, date,
    ...
    static function count($rules) {
        $query = '
            ...
            FROM
            ' . self::$table . self::addRelationQuery() . self::addFilterQuery($rules);
    ...
    private static function addFilterQuery($rules) {
        ...
        if (is_string($rules['keyword'])) {
            $query .= $hasBeenFiltered ? ' AND (employees.name LIKE :keyword OR employees.registrationNumber LIKE :keyword OR employees.email LIKE :keyword OR employees.phone LIKE :keyword OR designations.name LIKE :keyword)'
                                    : ' WHERE (employees.name LIKE :keyword OR employees.registrationNumber LIKE :keyword OR employees.email LIKE :keyword OR employees.phone LIKE :keyword OR designations.name LIKE :keyword)';

            $hasBeenFiltered = 1;
        }

        if (is_string($rules['startCreatedAt']) && is_string($rules['endCreatedAt'])) {
    ...
    private static function addRelationQuery() {
        return '
            ...
            INNER JOIN
                ' . Designation::$table . '
            ON
                ' . self::$table . '.designationId = ' . Designation::$table . '.id
        ';
    }
