...
{
    static $table = 'resignations',
            $columns = 'id, employeeId, noticeDate, date,
    ...
    private static function addFilterQuery($rules) {
        ...
        if (is_numeric($rules['startNoticeDate']) && is_numeric($rules['endNoticeDate'])) {
            $query .= $hasBeenFiltered ?
                        ' AND (' . self::$table . '.date BETWEEN ' . $rules['startNoticeDate'] . ' AND ' . $rules['endNoticeDate'] . ')' :
                        ' WHERE (' . self::$table . '.date BETWEEN ' . $rules['startNoticeDate'] . ' AND ' . $rules['endNoticeDate'] . ')';
